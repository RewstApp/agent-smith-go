...
func TestIsNewerVersion(t *testing.T) {
	tests := []struct {
		...
		{name: "bare current vs tagged latest", current: "1.2.3", latest: "v1.2.4", want: true},
		{name: "tagged current vs bare latest", current: "v1.2.3", latest: "1.2.4", want: true},
		{
			name:    "bare current equals tagged latest",
			current: "1.2.3",
			latest:  "v1.2.3",
			want:    false,
		},
		{
			name:    "default placeholder is older than any release",
			current: "v0.0.0",
			latest:  "v1.0.0",
			want:    true,
		},
