# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## Project Overview

Agent Smith is a lean, open-source command executor that integrates with Rewst workflows. It's written in Go and designed to run as a system service on Windows, Linux, and macOS client systems. The agent connects to Azure IoT Hub via MQTT to receive and execute commands remotely.

## Build Commands

Use PowerShell 7+ for building on all platforms:

```powershell
# Build for current platform
./scripts/build.ps1

# Clean build artifacts  
./scripts/clean.ps1

# Generate test coverage report
./scripts/coverage.ps1
```

The build script automatically detects the platform and creates platform-specific binaries in the `./dist/` directory:
- Windows: `rewst_agent_config.win.exe`
- Linux: `rewst_agent_config.linux.bin` 
- macOS: `rewst_agent_config.mac-os.bin`

## Testing

```bash
# Run all tests
go test ./...

# Run tests with coverage
go test -coverprofile=coverage.out ./... && go tool cover -html=coverage.out
```

## Development Dependencies

Required tools:
- [commitizen](https://commitizen-tools.github.io/commitizen/): `pipx install commitizen`
- [go-winres](https://github.com/tc-hib/go-winres): `go install github.com/tc-hib/go-winres@latest` (Windows builds only)

## Architecture

### Core Components

- **cmd/agent_smith/**: Main application entry point with three operational modes:
  - Configuration mode: `--config-url --config-secret --org-id`
  - Service mode: `--config-file --log-file --org-id` 
  - Uninstall mode: `--uninstall --org-id`

  Config mode reads the config endpoint's response body through an
  `io.LimitReader` capped at `maxConfigResponseSize` (10 MiB,
  `cmd/agent_smith/config_context.go`) and aborts the install when a response
  runs past it, rather than parsing a truncated prefix — `configHTTPTimeout`
  bounds how long the request runs but not how many bytes a slow-but-connected
  sender can push in that window. This mirrors `maxInstallerDownloadSize` on the
  auto-update download path. See the README's "Bounded Config-Fetch Response"
  section.

  The install, update and uninstall paths never assume the old agent process has
  exited: after stopping the service they wait (bounded, 2 minutes, documented)
  for real exit signals — the service manager no longer reporting the service
  active, no process still executing the agent binary, and the executable no
  longer held open — and return as soon as the process is gone. Overrunning the
  deadline aborts before writing or deleting anything, leaves the installation
  intact, and restarts the service that was stopped so a failed update cannot
  leave an endpoint offline. The agent executable and config file are written to a
  temp file and atomically renamed into place, so a failed write leaves the
  previous file byte-identical. See the README's "Waiting for the Old Agent
  Process to Exit" section. Once the uninstall path reaches the point of deleting
  installed files, it removes the data, program and scripts directories
  independently (`removeInstallationDirectories`) rather than aborting on the
  first `RemoveAll` failure, and reports every path that could not be removed —
  a single locked file (AV scan, stale Windows handle) no longer orphans the
  directories behind it with the service registration already gone. See the
  README's "Completing Uninstall When a Directory Cannot Be Removed" section. On Linux, the `--update` helper that performs this
  flow is launched in its own transient systemd scope
  (`cmd/agent_smith/run_command_linux.go`, `systemd-run --scope --collect`)
  rather than as a plain child of the running service, so it is never a member
  of that service's cgroup — otherwise `systemctl stop`'s default
  `KillMode=control-group` kills the helper along with the service it just
  stopped, before it can restart it, leaving the endpoint offline with no
  automatic recovery. See the README's "Surviving Its Own systemd Stop
  (Linux)" section.

- **internal/agent/**: Device configuration, installation paths, and OS-specific host information. Auto-update installers are downloaded into `<data directory>/updates` (a `0700` directory the agent owns) rather than the shared system temp directory, and `SweepStaleInstallers` reclaims installer binaries older than 24 hours at service startup — from that directory and from the legacy temp location — so the binaries a detached installer necessarily leaves behind stop accumulating one per update. See the README's "Reclaiming Downloaded Installer Binaries" section. The auto-update retry schedule is capped (1 hour, or a quarter of the check interval when shorter) and jittered (±25%) via `utils.JitteredBackoff`, the same helper the postback retry schedule uses, so the doubling cannot overflow into a negative sleep that busy-spins and a fleet-wide release-endpoint outage cannot produce a synchronized retry storm. See the README's "Capped and Jittered Auto-Update Retries" section. Before an installer is trusted, `Download` verifies it against the SHA-256 digest GitHub's Releases API returns natively for that asset (`Asset.Digest`, computed by GitHub itself at upload time — not a hand-published sidecar file), and `Run` gates the update on `isNewerVersion` — a proper semantic version comparison, not a not-equal check — so a corrupted or tampered download, or a release process mistake that republishes an older tag, can no longer replace the running agent. Both failures, and an installer over the 200 MiB size ceiling, abort the update rather than falling back to running the binary unverified. See the README's "Verified, Version-Gated Auto-Updates" section. On Windows, `host_windows.go`'s `DomainInfoProvider` bounds every `sc query` call (`IsEntraConnectServer`), the `dsregcmd /status` call (`EntraDomain`), and the WMI queries the PowerShell runner issues (`ADDomain`, `IsADDomainController`) with an internally enforced, `-ldflags`-overridable timeout (`hostCommandTimeout`, 30 seconds) rather than trusting the caller's context to carry a deadline, so a wedged SCM or a broken WMI repository — especially on domain controllers — can no longer hang config/install/update/uninstall's host-info gathering indefinitely. `IsEntraConnectServer` is the one field that shells out more than once (it probes four candidate service names): all four `sc query` calls share a single `hostCommandTimeout` budget rather than each getting its own, so the field is bounded by 30 seconds rather than four times that, and a probe that fails with the budget expired or the caller canceled aborts the whole check with an error naming the service and the bound instead of moving to the next name and returning a made-up `false` — a timeout is no longer silently reported as "not an Entra Connect server". This is the bound that keeps a `get_installation` MQTT worker from being occupied by a wedged SCM; that path is not covered by the per-command execution timeout, which bounds only the interpreter's own command execution. The `sc query` invocation is injectable (`scQueryFunc`, defaulting to `defaultSCQuery`) like the PowerShell runner, so the timeout and cancellation paths are unit-testable without depending on real SCM state. See the README's "Bounded Service and Host-Info Shell-Outs" section.
- **internal/version/**: The build-time version stamp. `Version` is the release tag (`v1.5.7`; default `v0.0.0`, same shape) and `Number()` is the total, non-panicking bare-number form used by the `x-rewst-agent-smith-version` header and `AGENT_SMITH_VERSION`; the twin, startup log and `--diagnostic` keep the tag form. Tests pin every case to a literal — never derive an expectation from the expression under test. See the README's "Reporting the Agent Version" section.
- **internal/interpreter/**: Command execution engine supporting both PowerShell and Bash interpreters  
- **internal/mqtt/**: Azure IoT Hub MQTT client implementation with auto-reconnection
- **internal/service/**: Cross-platform service management utilities. On Windows, `Stop()` waits a bounded, documented deadline (5 minutes) for the service to reach `Stopped` and otherwise returns an error naming the service and last observed state, so a wedged service aborts an update/install/uninstall with an actionable message instead of hanging forever. See the README's "Bounded Windows Service Stop" section. The service side of that stop reports its own progress: `windowsRunner.Execute` acknowledges `svc.Stop`/`svc.Shutdown` with `svc.StopPending` (`CheckPoint` 1, 30-second `WaitHint`) before the runner starts draining, and republishes `StopPending` with an incrementing `CheckPoint` every `stopPendingCheckpointInterval` (10 seconds) until the runner returns — otherwise the service jumped from `Running` straight to `Stopped`, and a shutdown that legitimately spent minutes draining a command worker (up to `command_timeout_seconds`) or waiting on plugin shutdown was logged by the SCM as not responding (events 7009/7043). All of `Execute`'s status updates route through one `statusReporter` that enforces the SCM's lifecycle order: nothing is published after `Stopped` (so it stays exactly-once and last even if a checkpoint goroutine wakes late, and a late send cannot block on a channel the dispatcher no longer reads), and `Running` is dropped once a stop has been acknowledged — a stop control can arrive before startup finishes. See the README's "Reporting Stop Progress to the SCM" section. Every `launchctl` call on macOS (`service_darwin.go`) and `systemctl` call on Linux (`service_linux.go`) runs via `exec.CommandContext` under its own documented, `-ldflags`-overridable timeout (`launchctlTimeout`/`systemctlTimeout`, 5 minutes), so a wedged launchd or a D-Bus stall aborts with an actionable error instead of hanging install/update/uninstall indefinitely — the same failure class as the Windows `Stop()` fix, just on the adjacent platforms. See the README's "Bounded Service and Host-Info Shell-Outs" section.
- **internal/syslog/**: OS-specific system logging (Linux/macOS/Windows). On Linux and macOS the shared implementation lives in `syslog_unix.go` (the two files were byte-identical apart from whether the source is repeated in the message body, which is all `syslog_linux.go`/`syslog_darwin.go` still hold). Forwarding to `logger` runs synchronously on whatever goroutine is logging, so each call is bounded by `syslogCommandTimeout` (5 seconds) via `exec.CommandContext` and kills the whole process group on expiry — a surviving descendant holding the output pipe would keep `cmd.Wait` blocked despite the deadline; a 1s `WaitDelay` backs that up, since the group kill misses a descendant that escaped the group (`setsid`, or re-homed by an init system) while still holding the pipe. The forward is best effort: `Write` always performs the on-disk write and returns *its* `(n, err)`, so a syslog failure can no longer return `(0, err)` early and delete the line from the log file operators read after an incident (matching the Windows event-log writer, which has always ignored event-log errors and written the file). After a failure the agent stops shelling out for `syslogSuppressWindow` (1 minute) and then probes with one line, since `logger` runs once per log line and a 5-second stall on each one would crawl all agent logging; the failing and recovering transitions are each recorded once into the log file in hclog's line format. Log-level classification (`levelForLine` in `syslog.go`) is shared by all three platforms rather than duplicated per writer: each one used to match the bracket text itself and all three looked for `[WARNING]`, which hclog never emits (it writes `[WARN] `), so every agent warning was recorded as informational on Linux, macOS and Windows alike. See the README's "Bounded, Non-Fatal Syslog Forwarding" section.
- **plugins/**: Plugin loader using HashiCorp's go-plugin framework
- **shared/**: Plugin interfaces and RPC definitions

### Plugin System

Agent Smith uses a plugin architecture for extensible notifications. Plugins are separate executables that implement the `Notifier` interface via RPC. The system supports loading multiple plugins simultaneously and sends status notifications (AgentStarted, AgentStatus:Online, AgentStatus:Offline, etc.) to all loaded plugins.

Loaded plugins are supervised: a subprocess that exits or crashes is detected (by a periodic health check and on the notification path) and relaunched with backoff, and delivery failures increment counters and are logged once per failure transition. See the README's "Notification Plugin Supervision" section. The crash-only health check cannot see a plugin that is still alive but has deadlocked internally, so every `Notify` RPC call (`shared.NotifierRPC.Notify`) is additionally bounded by `shared.NotifyTimeout` (10s): a call that never returns is abandoned rather than blocking the calling worker forever, counted separately from other notify failures (`NotifierStats.NotifyTimeouts`) so a hang is distinguishable from a crash, and the still-alive subprocess is killed and relaunched on the existing backoff schedule. See the README's "Bounded Notification Plugin RPC Calls" section.

### Message Processing Flow

1. Agent connects to Azure IoT Hub via MQTT on topic `devices/{device_id}/messages/devicebound/#`. Auto-ack is disabled: a received command is written to `<data directory>/command_journal` (`cmd/agent_smith/command_journal.go`) *before* the PUBACK is sent, so an acknowledgement means durably accepted rather than buffered in memory. The acknowledgement cannot instead wait for execution — IoT Hub re-enqueues an unacknowledged C2D message after a fixed one-minute lock without counting toward the delivery limit, so a command running longer than that would be redelivered every minute. On each connection cycle the journal is replayed from a snapshot taken *before* the cycle connects (`snapshotJournal`) minus every key this process owns (`svc.owned`) - listing it after subscribing reported a command the cycle itself had just started as interrupted (sc-118021), and replaying an owned key would double-run a command the previous cycle's workers are still finishing. Command workers are service-scoped, not cycle-scoped (`startWorkers`/`stopWorkers`): a cycle ending for SAS renewal or a lost connection closes the queue but does not cancel running commands - until sc-118039 it did, killing any command spanning the daily renewal - and only a service stop cancels them, leaving the entry started for the next process to report: never-started entries execute, started entries are reported to the engine as `interrupted` rather than re-run (the de-duplication boundary), entries past the 1h max age (IoT Hub's default TTL) are reported as expired and discarded, and completed entries leave a 1h tombstone so a redelivery whose PUBACK was lost is acknowledged without executing. Journal write failure degrades to in-memory acceptance with a once-per-transition Error, never a refusal. See the README's "Message Delivery Guarantee" section. Every MQTT operation waits with a deadline rather than an open-ended `token.Wait()`, and the connect/subscribe waits are additionally interruptible by the service stop signal, so a broker that keeps the connection open but stops acknowledging control packets (a throttling hub, a half-open middlebox) produces a logged failure and a backed-off reconnect instead of a device that is connected but never subscribed and cannot be stopped. See the README's "Bounded MQTT Operations" section.
2. Receives JSON messages containing either `commands` (shell scripts) or `get_installation` (system info requests)
3. Executes commands using platform-appropriate interpreter (PowerShell on Windows, Bash on Unix). Every command runs under a per-command execution deadline (`command_timeout_seconds`, default 30 minutes, `internal/agent/device.go`'s `DefaultCommandTimeout`) rather than an opt-in one, so a hung command (infinite loop, blocked on stdin, stuck network call) can no longer occupy a worker forever and, with enough hangs, silently stall all command execution on the device; exceeding the deadline kills the command's full descendant tree — a process group on Unix, a Windows job object created with `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` on Windows (`internal/interpreter/proc_windows.go`), since Windows has no process-group equivalent and a plain `Process.Kill()` only reaps the immediate shell, leaving anything it spawned (`Start-Process`, an installer, a stuck helper) running — frees the worker, and posts back a result flagged `timed_out`. See the README's "Bounding per-command execution time" and "Killing the Full Process Tree on Windows" sections. Stdout and stderr are each captured through an independently bounded writer (`max_output_bytes`, default 10 MiB per stream) so a verbose script cannot OOM the agent; output past the ceiling is discarded and the result is flagged `truncated` with both byte counts. See the README's "Bounding per-command output size" section. On Linux/macOS the command script is written to `<data directory>/scripts` (an agent-owned, `0700` directory — not the shared system temp directory) rather than a subdirectory of shared OS temp, and `utils.EnsureSecureDir` re-asserts that ownership/mode on every command rather than trusting a pre-existing directory, so a local unprivileged user can no longer pre-plant it with permissive ownership; Windows deliberately keeps its historical `C:\RewstRemoteAgent\scripts\<orgId>` location (some customers' AV/EDR whitelisting is scoped to it, and it was never reachable by the same pre-plant attack), with `EnsureSecureDir` there only refusing a symlink/non-directory swap. The write-then-exec path also re-reads the script and verifies its contents are unchanged immediately before exec, aborting rather than running altered content. See the README's "Hardening the Command Scripts Directory" section.
4. Posts results back to Rewst engine at `https://{rewst_engine_host}/webhooks/custom/action/{post_id}`. Results that exhaust their in-line retry budget are spooled to disk and re-attempted per connection cycle. The flush distinguishes an unreachable engine (stop, retry everything later) from an entry the engine rejected (pass over it, keep delivering the rest), so one undeliverable result can no longer pin the queue until the entries behind it age out. A rejected entry carries a persisted attempt counter and is abandoned after a bounded number of rejections with its own drop reason and a plugin notification. See the README's "Command Result Delivery" section.

### Client System Deployment

The agent runs as a system service with these key files:
- Configuration file: Contains device credentials, MQTT endpoints, logging settings, and plugin configurations
- Log file: Application logs (with optional syslog integration). Written through `utils.RotatingFile` (`internal/utils/logrotate.go`): rotated in-process at `log_max_bytes` (10 MiB) keeping `log_max_files` (5) copies, so the on-disk footprint is bounded at `(files+1) × size` instead of growing for the life of the installation. The writer closes its handle before renaming so rotation works on Windows, and rotates only at a line boundary (forced at 2× the ceiling) so a chunked plugin-stderr line is never split across files. A rename blocked by another process — on Windows, only the detached `--update` helper's inherited stdout; the diagnostic viewer opens with `FILE_SHARE_DELETE` (`cmd/agent_smith/diagnostic_open_windows.go`) so it never blocks — degrades to appending with one WARN per failure/recovery transition. A rename that succeeds but cannot reopen is reported separately, once the file reopens, with the count of writes lost. The detached `--update` helper gets a fresh `OpenAppendHandle()` to inherit rather than the writer. Both this writer and the syslog forwarder format their synthetic WARN lines via `utils.HclogWarnLine`. See the README's "Bounding the agent log file on disk" section.
- Plugin executables: Located at paths specified in device configuration
- Service binary: Platform-specific executable installed as system service

## Commit Convention

Use commitizen for standardized commit messages:
```bash
# Stage changes then commit
cz commit
```

Version management follows semantic versioning via commitizen in `.cz.toml`.