...
- **internal/service/**: Cross-platform service management utilities. On Windows, `Stop()` waits a bounded, documented deadline (5 minutes) for the service to reach `Stopped` and otherwise returns an error naming the service and last observed state, so a wedged service aborts an update/install/uninstall with an actionable message instead of hanging forever. See the README's "Bounded Windows Service Stop" section. Every `launchctl` call on macOS (`service_darwin.go`) and `systemctl` call on Linux (`service_linux.go`) runs via `exec.CommandContext` under its own documented, `-ldflags`-overridable timeout (`launchctlTimeout`/`systemctlTimeout`, 5 minutes), so a wedged launchd or a D-Bus stall aborts with an actionable error instead of hanging install/update/uninstall indefinitely — the same failure class as the Windows `Stop()` fix, just on the adjacent platforms. See the README's "Bounded Service and Host-Info Shell-Outs" section.
